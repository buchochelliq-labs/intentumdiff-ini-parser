fn main() {
    ...
    if target == "wasm32-wasip2" {
        println!("cargo:rustc-link-lib=static=tree_sitter_ini");
        println!("cargo:rustc-link-search=native={}/lib", manifest);
        println!("cargo:rerun-if-changed=lib/libtree_sitter_ini.a");
        return;
    }
    ...
    cfg.file("src/parser.c").compile("tree_sitter_ini");
